...
bool BelovModel::CalculateRepair(double Dz)
{
	...
	Y[0] = falpha*fDose;    //modified by Alessandro 27/08  default= Y[0] = falpha
...
std::vector<double> BelovModel::Belov_odes_system(double t,std::vector<double> Y)
{
	...
	//
	//------------NHEJ--------------
	double K1 = 10.02;             // M-1*h-1   default was 11.052
	double Kmin1 = 6.6*1e-01;   // h-1      default was 6.59999*1e-04
	double K2 = 5.82*1e+05; // M-1*h-1 default was 18.8305*(1.08517-std::exp(-21.418/std::pow(fDz,1.822)))
	...
	double P2 = 7.21;            // h-1 default was 0.39192
	double Pmin2 = 2.7605512*1e+02;  // h-1  there's not in the paper. implemented was  2.7605512*1e+02
	double P3 = 1.37*1e+04;          // M-1*h-1
	double Pmin3 = 2.34;            // h-1
	double P4 = 5.52*1e-02;       // h-1   default was 3.588*1e-02
	double P5 = 1.20*1e+05;          // M-1*h-1
	double Pmin5 = 8.82*1e-05;      // h-1
	double P6 = 1.87*1e+05;       // M-1*h-1  defaukt was 1.54368*1e+06
	double Pmin6 = 1.55*1e-03;      // h-1
	double P7 = 21.36;              // h-1   default was 1.54368*1e+06
	double P8 = 1.20*1e+04;          // M-1*h-1
	double Pmin8 = 2.49*1e-04;      // h-1
	double P9 = 4.88*1e-01;            //h-1      default was 1.104
	double P10 = 7.20*1e-03;        // h-1
	double P11 = 6.06*1e-04;        // h-1
	double P12 = 2.76*1e-01;        // h-1
	//
	//------------SSA--------------
	double Q1 = 7.80*1e+03;       // M-1*h-1  paper= 7.80*1e+03   implemented was=1.9941*1e+05
	double Qmin1 = 1.71*1e-04;      // h-1
	double Q2 = 3.00*1e+04;       // M-1*h-1   default was 4.8052*1e+04
	double Q3 = 6*1e+03;             // M-1*h-1
	double Qmin3 = 6.06*1e-04;      // h-1
	double Q4 = 1.66*1e-06;         // h-1   paper=1.66*1e-06   implemented=1.62*1e-03
	...
	//
	// Scalling rate XX1
	double XX1 = 9.074*1e-07; // M   modified by Alessandro, using the fribroblast volume! default implemented was 9.19*1e-07 (value from the paper)  
	//
	...
	// ----- NHEJ ----------
	YP.push_back( fNirrep*falpha*fDose - k1*Y[0]*X1 + kmin1*Y[1] - p1*Y[0]*X1 + pmin1*Y[10]); // [DSB] modified by Alessandro 27/08/25. default was fNirrep - ...
	...
	YP.push_back( (k9*(Y[3] + Y[4] + Y[5] + Y[6] + Y[7])*X6)/(k10 + Y[3] + Y[4] + Y[5] 
			+ Y[6] + Y[7]) - k11*Y[8] - k12*Y[9]); // [gH2AX foci] Y[9] (X6 è la concentrazione di H2AX)  

	// ----- HR ---------- 
	YP.push_back( p1*Y[0]*X7 - pmin1*Y[10] - p3*Y[10]*Y[11] + pmin3*Y[12]); 
	// [MRN/CtIP/ExoI/Dna2]     Y[10]
