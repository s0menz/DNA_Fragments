...
//   Defines the type of damage ollowing SDD formalism 
  ...
  unsigned int GetClusterChromo() const{return fChromo;};  //added by Ale. method to get the chromosome ID to a cluster
  
  //added by Alessandro. --> try to compute Fragment sizes in Bp units 
       static std::vector<ClassifiedDamage> FragmentSize(const std::vector<ClassifiedDamage>& listClassifiedDamage);
  ...
  // I didnt use these
  unsigned long int countDSB;
   unsigned long int fFragmentSizeBp;
  unsigned long int GetFragmentSizeBp() const{return fFragmentSizeBp;};
 
  // dichiaro il metodo che ordina il vettore dsbClusters
  static void SortDsbClusters(std::vector<ClassifiedDamage>& dsbClusters);
  //vector for the remaining fragments
...
private:
  ...
  unsigned int      fChromo;   //added by Ale to get Chromosome ID of the clusters
   
   std::vector<ClassifiedDamage> listClassifiedDamage;             //add by Alessandro. used for the method FragmentSize
