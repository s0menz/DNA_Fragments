...
void AnalysisHandler::GetAllDamageAndScanSB()
{
    ...
    //-------added by Ale. declaration of vector for all the dsb clusters. --> to sort the clusters based on the Bp 
    std::vector<ClassifiedDamage> allDsbClusters;
    // ------
    ...
    for (const auto& [chromo,evtDm] : dmMap) {
        for (const auto& [evt, dmV] : evtDm) {
			...
			//added by Ale.  fill the allDsbClusters vector
			allDsbClusters.insert(allDsbClusters.end(),
                              classifiedDamage.begin(),
                              classifiedDamage.end());
			// -------
    ...
    //added by Ale --->  call the two methods to create the dsbClusters vector and sort them
    std::vector<ClassifiedDamage> dsbClusters = ClassifiedDamage::FragmentSize(allDsbClusters);
    ClassifiedDamage::SortDsbClusters(dsbClusters);
